...
if args.mode in ["train","both"]:
    ...
    trainer = Trainer.from_params(model=model,
                                ...
                                params=trainer_params.duplicate())
    trainer.train()
    print("shape data train: ", train_dataset)
    # evaluate on the test set
    if test_dataset:
        logging.info("Evaluating on the test set")
        import torch  # import here to ensure the republication of the experiment
...
if args.mode in ["test","both"]:
    ...
    test_outputs = predictor.predict_multi_test(file_path = os.path.join("data",args.dataset,"test") ,batch_size = args.batch_size)
    # test = predictor.predict_multi_test(file_path = os.path.join("data",args.dataset,"test") ,batch_size = args.batch_size)
    ...
    # show on display
    predict = pd.Series(test_outputs["predict_labels"])
    true_labels = pd.Series(test_outputs["true_labels"])
    tokens = pd.Series(sum(pd.DataFrame(test_outputs["tokens"])["tokens"].values.tolist(),[]))

    # print("predict: " + str(predict))
    # print("len predict: "+ str(len(predict)))
    # print("true labels: "+ str(true_labels))
    # print(" len true labels: "+ str(len(true_labels)))
    ...
    df_test_y_ns = pd.Series(test_y_ns)
    # print("Da them ns: "+str(df_test_y_ns))
    # print('so luong label sau ns: ' + str(len(df_test_y_ns)))


    df_show = pd.concat([tokens, true_labels,predict, df_test_y_ns],axis=1)
    df_show.columns = ['tokens', 'true_labels', 'predict', 'predict_GDA']
